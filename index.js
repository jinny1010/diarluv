...
const SettingsApp = {
    ...
    render(charName) {
        ...
        <div id="settings-reset-modal" style="display:none;position:absolute;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:100;align-items:center;justify-content:center;">
            <div style="background:#2a2a2a;padding:20px;border-radius:12px;max-width:320px;width:90%;color:#fff;">
                ...
                <div id="settings-reset-app-list" style="max-height:250px;overflow-y:auto;"></div>
                <div style="display:flex;gap:10px;margin-top:15px;">
                    <button id="settings-reset-cancel" class="btn-secondary" style="flex:1;">취소</button>
                    <button id="settings-reset-confirm" class="btn-primary" style="flex:1;background:#e74c3c;">삭제</button>
                </div>
            </div>
        </div>`;
    },
    ...
    renderMain(data, charName, ddayData) {
        ...
        <div class="card" style="margin-top:15px;">
            <div class="card-label">🗑️ 데이터 초기화</div>
            <div class="settings-info-text" style="margin-bottom:10px;">
                현재 캐릭터의 앱 데이터를 선택하여 삭제합니다.
            </div>
            <button class="btn-primary" id="settings-reset-btn" style="background:#e74c3c;">🗑️ 앱 데이터 초기화</button>
        </div>`;
    },
    ...
    bindEvents(Core) {
        ...
        document.getElementById('settings-reset-btn')?.addEventListener('click', () => {
            ...
            for (const [id, info] of Object.entries(appInfo)) {
                ...
                listHtml += `
                    <label style="display:flex;align-items:center;gap:10px;padding:8px;border-radius:6px;cursor:pointer;${hasData ? '' : 'opacity:0.5;'}">
                        <input type="checkbox" class="settings-reset-checkbox" data-prefix="${info.prefix}" ${hasData ? '' : 'disabled'}>
            ...
            document.getElementById('settings-reset-app-list').innerHTML = listHtml;
        ...
        document.getElementById('settings-reset-modal')?.addEventListener('click', function(e) {
            if (e.target === this) this.style.display = 'none';
        });

        document.getElementById('settings-reset-confirm')?.addEventListener('click', () => {
            const selectedApps = [];
            document.querySelectorAll('.settings-reset-checkbox:checked').forEach(cb => {
                const prefix = cb.dataset.prefix;
                const appName = cb.closest('label').querySelector('span').textContent;
